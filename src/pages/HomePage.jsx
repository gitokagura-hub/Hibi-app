...
const accentMap = {
  black: { border: "border-gray-900", iconBg: "bg-app-raised", iconText: "text-ink", badgeBg: "bg-app-raised", badgeText: "text-ink-sub" },
    ...
    <div className="mb-4">
      <button
        ...
        className={`w-full flex items-center gap-3 rounded-2xl border p-3.5 text-left ${
          connected ? "bg-emerald-50 border-emerald-100" : "bg-app-surface border-app-line active:scale-[0.98]"
        }`}
      >
        <div
          className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
            connected ? "bg-emerald-100" : "bg-app-raised"
          }`}
        >
          <HardDrive size={16} className={connected ? "text-emerald-700" : "text-ink-sub"} />
        </div>
        ...
        {connected && (
          <button
            ...
            className="text-xs text-ink-sub px-2"
          >
            解除
          </button>
        )}
...
export default function HomePage({ onSelect }) {
  return (
    <div className="min-h-screen bg-app-bg flex flex-col">
      ...
      <header className="bg-app-surface sticky top-0 z-10">
        <div className="px-5 pt-14 pb-3">
          <h1 className="text-3xl font-semibold tracking-tight">Home</h1>
          <p className="mt-1 text-sm text-ink-sub">Gito's Workspace</p>
        </div>
      </header>
          ...
          {apps.map((item) => {
            ...
            return (
              <button
                ...
                className={`text-left bg-app-surface rounded-2xl border border-app-line border-l-4 ${a.border} p-5 active:scale-[0.98] transition-transform`}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="mt-1 text-sm text-ink-sub leading-relaxed">{item.description}</p>
